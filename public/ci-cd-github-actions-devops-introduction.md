...
- 自分のリポジトリに `.github/workflows/ci.yml` を作り、上の最小CIを貼って push する
- `npm test` が無い場合は、まず `npm run lint`（または `npm run build`）に置き換えて通す
- あえて失敗させて、Actions のログで「どの Step で落ちたか」を確認する（ログ耐性が一気に付く）
